...
public class EventEndpoint {
    ...
    public Response getDSOproperty(@PathParam("property") String property) {      
        return Response.ok(config.getProperties().getProperty(property)).build();
    }

    @GET
    @Path("/setProperty/{property}/{value}")
    public Response setDSOproperty(@PathParam("property") String property,@PathParam("value") String value) {
        try {
        config.storeProperties(property,value);  
        }  
        catch (IOException  e) {
            LOGGER.warn("Cannot write to properties file", e);
        }   
        return Response.status(Response.Status.OK).build();
    }
