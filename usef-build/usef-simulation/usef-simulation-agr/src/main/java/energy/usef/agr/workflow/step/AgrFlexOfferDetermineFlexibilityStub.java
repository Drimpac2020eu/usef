...
public class AgrFlexOfferDetermineFlexibilityStub implements WorkflowStep {
    ...
    public WorkflowContext invoke(WorkflowContext context) {
        ...
        for (FlexRequestDto flexRequestDto : inputFlexRequests) {
            ...
            String  tmp2 = ConfigAgr.getConfigurationFolder() + "f" + connectionGroup.replaceAll(":", ".") + ".json";
            LOGGER.info(tmp2);
          //  File file = new File(tmp2);
 List<energy.usef.core.model.Connection>  _connections =  corePlanboardBusinessService.findConnectionsForConnectionGroup(flexRequestDto.getConnectionGroupEntityAddress(), flexRequestDto.getPeriod());

        String eanIDs= "?congestionID=";
eanIDs += flexRequestDto.getConnectionGroupEntityAddress();
...
new javax.net.ssl.HostnameVerifier(){
    ...
    //   String url = "https://160.40.49.244:9100/drimpac-aggregator/rest/api/v1/get_flexibility_offer" + eanIDs;
        String url = "https://localhost:9000/drimpac-aggregator/rest/api/v1/get_flexibility_offer" + eanIDs;
...
System.out.println(temp2);          
            ...
            //PtuOfferJson[] ptuJson = objectMapper.readValue(file, PtuOfferJson[].class);  //Drimpac
            PtuOfferJson[] ptuJson = objectMapper.readValue(temp2, PtuOfferJson[].class);  //Drimpac
          ...
          //      } else {
           ...
           //     }
            for (int i = 0; i < ptuJson.length; i++) { 
                PtuFlexOfferDto ptuFlexOfferDto = new PtuFlexOfferDto();
                ptuFlexOfferDto.setPower(BigInteger.valueOf(ptuJson[i].getPower()));
                ptuFlexOfferDto.setPrice(BigDecimal.valueOf(ptuJson[i].getPrice()));
                ptuFlexOfferDto.setPtuIndex(BigInteger.valueOf(ptuJson[i].getStart()));
        ...
        //return context;
              ...
              LOGGER.info("Can't find flexoffer file");
             e.printStackTrace();
